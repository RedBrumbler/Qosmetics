...
namespace NoteUtils
{
    ...
    {
        if (!mesh || !config.noteConfig.overrideNoteSize) return;
    ...
    void HideBaseGameNotes(Transform* noteCube, Qosmetics::NoteConfig& config, bool hide)
    {
        ...
        if(config.get_disableBaseGameArrows())
        {
            ...
            if (!NoteArrow) NoteArrow = il2cpp_utils::createcsstr("NoteArrow", il2cpp_utils::StringType::Manual);
            if (!NoteArrowGlow) NoteArrowGlow = il2cpp_utils::createcsstr("NoteArrowGlow", il2cpp_utils::StringType::Manual);
            if (!NoteCircleGlow) NoteCircleGlow = il2cpp_utils::createcsstr("NoteCircleGlow", il2cpp_utils::StringType::Manual);

            if (Transform* noteArrow = noteCube->Find(NoteArrow)) noteArrow->get_gameObject()->SetActive(false);
            if (Transform* noteArrowGlow = noteCube->Find(NoteArrowGlow)) noteArrowGlow->get_gameObject()->SetActive(false);
            if (Transform* noteCircleGlow = noteCube->Find(NoteCircleGlow)) noteCircleGlow->get_gameObject()->SetActive(false);
        }
        ...
        if (cubeFilter) cubeFilter->set_mesh(nullptr);
    }
    ...
    void SetDebrisProperties(GameObject* debris, Vector3 cutPoint, Vector3 cutNormal)
    {
        ...
        for (int i = 0; i < rendererCount; i++)
        {
            ...
            if (!_SlicePos) _SlicePos = Shader::PropertyToID(il2cpp_utils::createcsstr("_SlicePos"));
            if (!_CutPlane) _CutPlane = Shader::PropertyToID(il2cpp_utils::createcsstr("_CutPlane"));
            if (!_TransformOffset) _TransformOffset = Shader::PropertyToID(il2cpp_utils::createcsstr("_TransformOffset"));

            if (material->HasProperty(_SlicePos)) material->SetVector(_SlicePos, slicePos);
            if (material->HasProperty(_CutPlane)) material->SetVector(_CutPlane, cutPlane);
            if (material->HasProperty(_TransformOffset)) material->SetVector(_TransformOffset, transformOffset);
    ...
    bool anyCC(Renderer* renderer)
    {
        ...
        for (int i = 0; i < materialCount; i++)
        {
            ...
            if (materialName.find("_replace") != std::string::npos && MaterialUtils::ShouldCC(materials->values[i], true)) return true;
        }
