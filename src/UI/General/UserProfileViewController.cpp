...
namespace Qosmetics::UI
{
    void UserProfileViewController::DidActivate(bool firstActivation, bool addedToHierarchy, bool screenSystemEnabling)
    {
        if (firstActivation)
        {
            ...
            text = BeatSaberUI::CreateText(layout->get_transform(), "While it works, it's very experimental, changes are not");
            text->set_alignment(TMPro::TextAlignmentOptions::Center);
            layoutelem = UnityUtils::GetAddComponent<LayoutElement*>(text->get_gameObject());
            layoutelem->set_preferredHeight(height);
            layoutelem->set_preferredWidth(70.0f);
            text = BeatSaberUI::CreateText(layout->get_transform(), "always correctly reflected in the settings UI.");
            text->set_alignment(TMPro::TextAlignmentOptions::Center);
            layoutelem = UnityUtils::GetAddComponent<LayoutElement*>(text->get_gameObject());
            layoutelem->set_preferredHeight(height);
            layoutelem->set_preferredWidth(70.0f);
            text = BeatSaberUI::CreateText(layout->get_transform(), "A restart Will almost definitely always fix this.");
            text->set_alignment(TMPro::TextAlignmentOptions::Center);
            layoutelem = UnityUtils::GetAddComponent<LayoutElement*>(text->get_gameObject());
            layoutelem->set_preferredHeight(height);
            layoutelem->set_preferredWidth(70.0f);
